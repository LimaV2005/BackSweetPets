...
module.exports = {
  ...
  try {
    const produto = await listadesejos.findOne({ where: { id_produto: id_produto, id_usuario: id_usuario} });
    const nome_produto = await Produto.findOne({where: {id: id_produto}})
    if (produto) {
      const nome = nome_produto.nome_produto;
      await listadesejos.destroy({ where: { id_produto: id_produto, id_usuario: id_usuario}  });
